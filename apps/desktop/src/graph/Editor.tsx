...
import { NodeBody, type BodyData, type NodeState } from './Node.tsx';
...
function Board({ registry, library, doc: initial, onDocChange }: EditorProps): React.ReactElement {
    ...
    setFlowNodes((prev) => {
      ...
      return doc.nodes.map((n) => {
        ...
        const data: BodyData = {
          ...
          onParams: (patch) => setParams(n.id, patch),
          onPick: (key, what) => void pick(n.id, key, what),
        };
        const old = before.get(n.id);
        return {
          // Spread first so the measurement survives, then overwrite what the
          // document owns.
          ...old,
          id: n.id,
          type: 'dialect',
          position: old?.position ?? n.at ?? { x: 0, y: 0 },
          data: data as unknown as Record<string, unknown>,
          selected: selected === n.id,
        };
      });
    });
  }, [doc.nodes, runs, world, selected, setParams, pick]);
  ...
  const onNodesChange = useCallback(
    (changes: NodeChange[]): void => {
      // Every change goes to the view first — including the dimension changes
      // that make a node visible at all.
  ...
  const refreshSaved = useCallback((): void => {
    void listGraphs().then(setSaved).catch(() => setSaved([]));
  }, []);
